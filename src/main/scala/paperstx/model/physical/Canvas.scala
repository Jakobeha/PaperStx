...
import paperstx.model.block.{Scope, TypedBlock}

import scala.scalajs.js.Dynamic.global

/**
  * Like a source file, contains code in block form -
  * specifically, in the form of [[paperstx.model.block.Blob]]s.
...
case class Canvas(otherExprs: List[PhysBlob], selection: Option[Selection]) {
  ...
  /** Contains blocks derived from the expressions. */
  def bindBlocks(scope: Scope): Seq[TypedBlock] =
    allExprs.flatMap(_.bindBlocks(scope))

  /** The next available uid for a physical blob. */
  lazy val nextUid: Int = nextUidFrom(0)

  /** The next available uid for a physical blob, greater than or equal to the given id. */
  private def nextUidFrom(curUid: Int): Int = {
    if (otherExprs.forall(_.uid != curUid)) {
      curUid
    } else {
      nextUidFrom(curUid + 1)
    }
  }
